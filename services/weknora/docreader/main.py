...
from docreader.memory_manager import memory_manager
    ...
    def ReadFromFile(self, request: ReadFromFileRequest, context):
        ...
        with request_id_context(request_id):
            try:
                ...
                # 内存管理：处理完成后检查并清理内存
                memory_manager.check_and_cleanup(f"ReadFromFile:{request.file_name}")
                ...
                # 内存管理：出错时也要清理
                memory_manager.force_cleanup(f"ReadFromFile:ERROR")
                context.set_code(grpc.StatusCode.INTERNAL)
    ...
    def ReadFromURL(self, request: ReadFromURLRequest, context):
        ...
        with request_id_context(request_id):
            try:
                ...
                # 内存管理：处理完成后检查并清理内存
                memory_manager.check_and_cleanup(f"ReadFromURL:{request.url}")
                ...
                logger.info(f"Detailed traceback: {traceback.format_exc()}")
                # 内存管理：出错时也要清理
                memory_manager.force_cleanup(f"ReadFromURL:ERROR")
                context.set_code(grpc.StatusCode.INTERNAL)
                context.set_details(str(e))
                return ReadResponse(error=str(e))
