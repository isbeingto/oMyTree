...
class DocReaderConfig:
    ...
    pdf_ocr_min_unique_ratio: float
...
def load_config() -> DocReaderConfig:
    ...
    # PDF OCR fallback
    pdf_ocr_min_text_chars = _get_int(
        ["DOCREADER_PDF_OCR_MIN_TEXT_CHARS", "PDF_OCR_MIN_TEXT_CHARS"], 200
    )
    pdf_ocr_min_unique_ratio = _get_float(
        ["DOCREADER_PDF_OCR_MIN_UNIQUE_RATIO", "PDF_OCR_MIN_UNIQUE_RATIO"], 0.12
    )
    pdf_ocr_max_pages = _get_int(
        ["DOCREADER_PDF_OCR_MAX_PAGES", "PDF_OCR_MAX_PAGES"], 50
    )
    pdf_ocr_render_scale = _get_float(
        ["DOCREADER_PDF_OCR_RENDER_SCALE", "PDF_OCR_RENDER_SCALE"], 2.0
    )
    ...
    # Local storage
    local_storage_base_dir = _get_str(
        ["DOCREADER_LOCAL_STORAGE_BASE_DIR", "LOCAL_STORAGE_BASE_DIR"], "./data/files"
    )
    ...
    return DocReaderConfig(
        ...
        pdf_ocr_min_unique_ratio=pdf_ocr_min_unique_ratio,
        pdf_ocr_max_pages=pdf_ocr_max_pages,
        pdf_ocr_render_scale=pdf_ocr_render_scale,
...
def dump_config(mask_secrets: bool = True) -> Dict[str, Any]:
    ...
    d: Dict[str, Any] = {
        ...
        # PDF OCR fallback
        "DOCREADER_PDF_OCR_MIN_TEXT_CHARS": cfg.pdf_ocr_min_text_chars,
        "DOCREADER_PDF_OCR_MIN_UNIQUE_RATIO": cfg.pdf_ocr_min_unique_ratio,
        "DOCREADER_PDF_OCR_MAX_PAGES": cfg.pdf_ocr_max_pages,
        "DOCREADER_PDF_OCR_RENDER_SCALE": cfg.pdf_ocr_render_scale,
        # VLM
