...
class PaddleOCRBackend(OCRBackend):
    ...
    def __init__(self):
        ...
        try:
            ...
            det_side_len = max(256, min(det_side_len, 1600))

            # Prefer lighter-weight models by default to keep RSS stable on small VMs.
            rec_model = (
                "PP-OCRv4_mobile_rec" if model_size == "mobile" else "PP-OCRv4_server_rec"
            )
            det_model = (
                "PP-OCRv4_mobile_det" if model_size == "mobile" else "PP-OCRv4_server_det"
            )
            ...
            ocr_config = {
                ...
                # These extras can increase memory/latency; keep them off by default.
            ...
            logger.info(
                "PaddleOCR engine initialized successfully (model_size=%s, det_side_len=%s)",
                model_size,
                det_side_len,
            )
