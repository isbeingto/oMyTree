...
# ============================================================================
# Lazy imports for python-docx to reduce memory footprint at module load time.
# This allows OCR subprocesses to avoid loading the heavy python-docx library.
# ============================================================================
_docx_Document = None
_docx_exceptions = None


def _get_docx_Document():
    """Lazy import python-docx Document class."""
...
def _get_docx_exceptions():
    """Lazy import python-docx image exceptions."""
    global _docx_exceptions
    if _docx_exceptions is None:
        ...
        _docx_exceptions = {
            "InvalidImageStreamError": InvalidImageStreamError,
            "UnexpectedEndOfFileError": UnexpectedEndOfFileError,
            "UnrecognizedImageError": UnrecognizedImageError,
        }
    return _docx_exceptions


# Provide backward-compatible names at module level (resolved lazily)
def Document(*args, **kwargs):
    """Backward-compatible wrapper for docx.Document (lazy loaded)."""
    return _get_docx_Document()(*args, **kwargs)


class _LazyExceptionMeta(type):
    """Metaclass for lazy-loaded exceptions that still work with isinstance/except."""
    pass


# We can't truly lazy-load exception classes for `except` clauses easily,
# so we import them on first access via __getattr__ at module level.
def __getattr__(name):
    if name in ("InvalidImageStreamError", "UnexpectedEndOfFileError", "UnrecognizedImageError"):
        return _get_docx_exceptions()[name]
...
class Docx:
    ...
    def get_picture(self, document, paragraph) -> Optional[Image.Image]:
        ...
        # Get lazy-loaded exception classes
        exc = _get_docx_exceptions()
        UnrecognizedImageError = exc["UnrecognizedImageError"]
        UnexpectedEndOfFileError = exc["UnexpectedEndOfFileError"]
        InvalidImageStreamError = exc["InvalidImageStreamError"]
