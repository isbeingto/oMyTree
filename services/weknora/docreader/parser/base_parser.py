...
from docreader.ocr.isolated import get_isolated_client, is_isolation_enabled
from docreader.ocr.queue import get_ocr_queue, is_queue_enabled
...
class BaseParser(ABC):
    ...
    def __init__(
        ...
        ocr_config: dict = {},
        max_image_size: int = None,  # Maximum image size, read from env if None
        ...
        **kwargs,
        ...
        # Use env var if max_image_size not explicitly set
        if max_image_size is None:
            max_image_size = int(os.getenv("DOCREADER_IMAGE_MAX_SIZE", "1280"))
    ...
    def perform_ocr(self, image: Image.Image):
        ...
        use_isolated = is_isolation_enabled() and backend == "paddle"
        use_queue = is_queue_enabled()

        # Execute OCR prediction
        logger.info(
            "Executing OCR prediction (backend=%s, isolated=%s, queued=%s)",
            backend,
            use_isolated,
            use_queue,
        )

        if use_queue and use_isolated:
            # 使用队列化处理，确保OCR任务一个一个排队执行
            ocr_result = self._perform_ocr_queued(resized_image, backend)
        elif use_isolated:
            ocr_result = get_isolated_client(backend).predict(resized_image)
        else:
            # Get OCR engine
            ocr_engine = OCREngine.get_instance(backend)
    ...
    def _perform_ocr_queued(self, image: Image.Image, backend: str) -> str:
        """通过队列执行OCR，确保任务串行处理。
        ...
        def do_ocr(img_bytes: bytes) -> str:
            ...
            result = get_isolated_client(backend).predict(img)
            try:
                img.close()
            except Exception:
                pass
        ...
        queue = get_ocr_queue()
        logger.info(
            "[OCR Queue] Submitting task %s, queue_status=%s",
            task_id, queue.get_status()
        )
        ...
        try:
            ...
            logger.error("[OCR Queue] Task %s failed: %s", task_id, e)
            # 如果队列失败，回退到直接调用
            logger.warning("[OCR Queue] Falling back to direct OCR call")
            return get_isolated_client(backend).predict(image)
    ...
    def _resize_image_if_needed(self, image: Image.Image) -> Image.Image:
        ...
        # Bound total pixels to avoid large intermediate numpy arrays.
        max_pixels = int(os.getenv("DOCREADER_IMAGE_MAX_PIXELS", "20000000") or "20000000")
        target_scale = 1.0
        if max_pixels > 0 and (width * height) > max_pixels:
            target_scale = (max_pixels / float(width * height)) ** 0.5

        # Bound longest side.
        if width > self.max_image_size or height > self.max_image_size:
            target_scale = min(target_scale, self.max_image_size / width, self.max_image_size / height)

        if target_scale < 0.999:
            new_width = max(1, int(width * target_scale))
            new_height = max(1, int(height * target_scale))
            logger.info(
                "Resizing PIL image: %sx%s -> %sx%s (max_side=%s, max_pixels=%s)",
                width,
                height,
                new_width,
                new_height,
        ...
        logger.info(
            "PIL image size is %sx%s, no resizing needed (max_side=%s, max_pixels=%s)",
            width,
            height,
            self.max_image_size,
            max_pixels,
