...
func (e *OpenAIEmbedder) BatchEmbed(ctx context.Context, texts []string) ([][]float32, error) {
	...
	if resp.StatusCode != http.StatusOK {
		logger.GetLogger(ctx).Errorf("OpenAIEmbedder EmbedBatch API error: Http Status %s, Body: %s", resp.Status, string(body))
		// Provide more specific error messages for common HTTP status codes
		switch resp.StatusCode {
		case http.StatusUnauthorized:
			return nil, fmt.Errorf("EmbedBatch API error: Http Status %s - API key is invalid or expired, please check your API key configuration", resp.Status)
		case http.StatusForbidden:
			return nil, fmt.Errorf("EmbedBatch API error: Http Status %s - access denied, please verify API key permissions", resp.Status)
		case http.StatusTooManyRequests:
			return nil, fmt.Errorf("EmbedBatch API error: Http Status %s - rate limit exceeded, please try again later", resp.Status)
		default:
