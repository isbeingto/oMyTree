...
func (t *Tenant) GetEffectiveEngines() []RetrieverEngineParams {
	if t != nil && len(t.RetrieverEngines.Engines) > 0 {
		filtered := make([]RetrieverEngineParams, 0, len(t.RetrieverEngines.Engines))
		for _, e := range t.RetrieverEngines.Engines {
			if strings.TrimSpace(string(e.RetrieverEngineType)) == "" {
				continue
			}
			if strings.TrimSpace(string(e.RetrieverType)) == "" {
				continue
			}
