...
func NewAsynqServer() *asynq.Server {
	...
	// Get concurrency from environment variable, default to 1 for memory safety
	// This limits how many document processing tasks run in parallel
	// to prevent OOM when multiple large files are uploaded simultaneously
	concurrency := 1
	if envConcurrency := os.Getenv("ASYNQ_CONCURRENCY"); envConcurrency != "" {
	...
	srv := asynq.NewServer(
		opt,
		asynq.Config{
			// CRITICAL: Limit concurrency to prevent OOM when processing multiple documents
			// Each document processing task loads entire file content into memory
			// With concurrency=1, files are processed serially (one at a time)
