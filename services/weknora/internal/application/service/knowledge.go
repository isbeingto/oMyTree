...
func (s *knowledgeService) CreateKnowledgeFromFile(ctx context.Context,
	...
	// 说明：VLM/多模态已移除，所有文件（包括图片）都走本地解析链路（OCR 等）。
	if IsImageType(getFileType(fileName)) {
		logger.Info(ctx, "Image upload detected, will use local parsing (OCR)")
	}
	...
	if strings.TrimSpace(kb.EmbeddingModelID) == "" {
		modelID, err := s.getDefaultEmbeddingModelID(ctx)
		if err != nil {
			return nil, err
		}
	...
	// 说明：VLM 已移除，所有文件走本地解析，EnableMultimodel 固定为 false
...
func (s *knowledgeService) DeleteKnowledge(ctx context.Context, id string) error {
	...
	wg.Go(func() error {
		...
		embeddingModelID := strings.TrimSpace(knowledge.EmbeddingModelID)
		if embeddingModelID == "" {
			logger.GetLogger(ctx).WithField("knowledge_id", knowledge.ID).Warn("DeleteKnowledge skip vector delete: empty embedding_model_id")
			return nil
		}
		embeddingModel, err := s.modelService.GetEmbeddingModel(ctx, embeddingModelID)
		if err != nil {
			// Model may be inactive or deleted. Try to get dimensions from model record directly
			// or skip vector deletion gracefully.
			logger.GetLogger(ctx).WithFields(map[string]interface{}{
				"error":              err,
				"embedding_model_id": embeddingModelID,
			}).Warn("DeleteKnowledge: embedding model unavailable, attempting to skip vector deletion gracefully")
			// Vector cleanup is best-effort; the model may have been deleted or deactivated.
			// We allow the deletion to proceed without failing the entire operation.
			return nil
		}
	...
	wg.Go(func() error {
		...
		for _, knowledge := range knowledgeList {
			if strings.TrimSpace(knowledge.EmbeddingModelID) == "" {
				logger.GetLogger(ctx).WithField("knowledge_id", knowledge.ID).Warn("DeleteKnowledgeList skip vector delete: empty embedding_model_id")
				continue
			}
		...
		for key, knowledgeIDs := range group {
			...
			if err != nil {
				// Model may be inactive or deleted. Skip vector deletion gracefully.
				logger.GetLogger(ctx).WithFields(map[string]interface{}{
					"error":              err,
					"embedding_model_id": key.EmbeddingModelID,
				}).Warn("DeleteKnowledgeList: embedding model unavailable, skipping vector deletion for this group")
				continue
			}
...
func (s *knowledgeService) getDefaultEmbeddingModelID(ctx context.Context) (string, error) {
	...
	// Selection priority:
	// 1. Tenant's own default embedding model (if usable)
	// 2. Any usable tenant-owned embedding model
	// 3. Global builtin embedding model (is_builtin=true, if usable)
	// 4. Any usable builtin embedding model
	// This ensures all tenants can fall back to the global builtin model when they don't
	// have their own usable embedding models configured.

	var tenantDefaultID, tenantFallbackID string
	var builtinDefaultID, builtinFallbackID string
	var anyActiveID string
	...
	for _, m := range models {
		...
		if !isUsableEmbeddingModel(m) {
			continue
		}

		// Categorize by builtin vs tenant-owned
		if m.IsBuiltin {
			if m.IsDefault && builtinDefaultID == "" {
				builtinDefaultID = m.ID
			} else if builtinFallbackID == "" {
				builtinFallbackID = m.ID
			}
		} else {
			if m.IsDefault && tenantDefaultID == "" {
				tenantDefaultID = m.ID
			} else if tenantFallbackID == "" {
				tenantFallbackID = m.ID
			}
		}
	}
	...
	if tenantDefaultID != "" {
		return tenantDefaultID, nil
	}
	if tenantFallbackID != "" {
		return tenantFallbackID, nil
	}
	if builtinDefaultID != "" {
		return builtinDefaultID, nil
	}
	if builtinFallbackID != "" {
		return builtinFallbackID, nil
	}

	if anyActiveID != "" {
		return "", errors.New("no usable active embedding model configured (check base_url/api_key)")
	}
	return "", errors.New("no active embedding model configured")
}
...
func (s *knowledgeService) processChunks(ctx context.Context,
	...
	embeddingModelID := strings.TrimSpace(kb.EmbeddingModelID)
	if embeddingModelID == "" {
		id, err := s.getDefaultEmbeddingModelID(ctx)
		if err != nil {
			knowledge.ParseStatus = types.ParseStatusFailed
			knowledge.ErrorMessage = err.Error()
			knowledge.UpdatedAt = time.Now()
			_ = s.repo.UpdateKnowledge(ctx, knowledge)
			logger.GetLogger(ctx).WithField("error", err).Errorf("processChunks resolve default embedding model failed")
			return
		}
		embeddingModelID = id
	...
	span.SetAttributes(
		...
		attribute.String("embedding_model_id", embeddingModelID),
	...
	// If the specified model is unavailable (inactive, not found, or misconfigured),
	// fallback to the global default embedding model.
	embeddingModel, err := s.modelService.GetEmbeddingModel(ctx, embeddingModelID)
	if err != nil {
		logger.Warnf(ctx, "Specified embedding model %s unavailable: %v, trying fallback to default", embeddingModelID, err)
		// Try fallback to default embedding model
		fallbackID, fallbackErr := s.getDefaultEmbeddingModelID(ctx)
		if fallbackErr != nil {
			knowledge.ParseStatus = types.ParseStatusFailed
			knowledge.ErrorMessage = fmt.Sprintf("specified model unavailable (%v) and no fallback: %v", err, fallbackErr)
			knowledge.UpdatedAt = time.Now()
			_ = s.repo.UpdateKnowledge(ctx, knowledge)
			logger.GetLogger(ctx).WithField("error", err).Errorf("processChunks no usable embedding model")
			span.RecordError(err)
			return
		}
		embeddingModel, err = s.modelService.GetEmbeddingModel(ctx, fallbackID)
		if err != nil {
			knowledge.ParseStatus = types.ParseStatusFailed
			knowledge.ErrorMessage = err.Error()
			knowledge.UpdatedAt = time.Now()
			_ = s.repo.UpdateKnowledge(ctx, knowledge)
			logger.GetLogger(ctx).WithField("error", err).Errorf("processChunks get fallback embedding model failed")
			span.RecordError(err)
			return
		}
		// Update to use the fallback model
		embeddingModelID = fallbackID
		kb.EmbeddingModelID = fallbackID
		knowledge.EmbeddingModelID = fallbackID
		logger.Infof(ctx, "Using fallback embedding model: %s", fallbackID)
		span.SetAttributes(attribute.String("embedding_model_id_fallback", fallbackID))
	}
	...
	if err == nil {
		...
		// 这里不阻断后续流程（避免因为清理失败而阻塞解析），但后续会再次初始化用于写入。
		logger.Warnf(ctx, "Failed to init retrieval engine for cleanup (will retry later): %v", err)
	}
	...
	// ========== DocReader 解析结果日志 ==========
	verboseChunks := false
	if v := strings.TrimSpace(os.Getenv("WEKNORA_DOCREADER_CHUNK_VERBOSE")); v != "" {
	...
	logger.Infof(ctx, "[DocReader] Parsed: knowledge_id=%s kb_id=%s chunks=%d (empty=%d) len[min=%d max=%d] chunks_with_images=%d total_images=%d verbose=%t",
		knowledge.ID,
	...
	// Avoid log explosions: only dump per-chunk details when explicitly enabled.
	if verboseChunks {
		for idx, chunkData := range chunks {
			...
			contentPreview = strings.ReplaceAll(contentPreview, "\r", "\\r")
			contentPreview = strings.ReplaceAll(contentPreview, "\n", "\\n")
			contentPreview = strings.ReplaceAll(contentPreview, "\t", "\\t")

			logger.Debugf(ctx, "[DocReader] Chunk #%d (seq=%d): len=%d images=%d range=[%d-%d] preview=%s",
				idx, chunkData.Seq, len(chunkData.Content), len(chunkData.Images), chunkData.Start, chunkData.End, contentPreview)

			for imgIdx, img := range chunkData.Images {
				logger.Debugf(ctx, "[DocReader]   Image #%d: url=%s original_url=%s pos=[%d-%d] ocr_len=%d caption_len=%d",
					imgIdx,
					img.Url,
					img.OriginalUrl,
					img.Start,
	...
	for _, chunkData := range chunks {
		...
		if len(chunkData.Images) > 0 {
			...
			for i, img := range chunkData.Images {
				...
				// 为图片OCR/Caption生成稳定且不冲突的 ChunkIndex（便于排序/调试）
				imageBaseIndex := maxSeq + 1 + int(chunkData.Seq)*1000
				...
				if img.Caption != "" {
					captionChunk := &types.Chunk{
						...
						KnowledgeBaseID: knowledge.KnowledgeBaseID,
						Content:         img.Caption,
						ChunkIndex:      imageBaseIndex + i*2 + 1,
						IsEnabled:       true,
						CreatedAt:       time.Now(),
						UpdatedAt:       time.Now(),
						StartAt:         int(img.Start),
	...
	span.AddEvent("init retrieval engine")
	if retrieveEngine == nil {
		retrieveEngine, err = retriever.NewCompositeRetrieveEngine(s.retrieveEngine, tenantInfo.GetEffectiveEngines())
		if err != nil {
			knowledge.ParseStatus = types.ParseStatusFailed
			knowledge.ErrorMessage = fmt.Sprintf("init retrieval engine failed: %v", err)
			knowledge.UpdatedAt = time.Now()
			_ = s.repo.UpdateKnowledge(ctx, knowledge)
			logger.GetLogger(ctx).WithField("error", err).Errorf("processChunks init retrieval engine failed")
			span.RecordError(err)
			return
		}
	}
	...
	batchSize := 100
	if v := strings.TrimSpace(os.Getenv("WEKNORA_INDEX_BATCH_SIZE")); v != "" {
		if n, convErr := strconv.Atoi(v); convErr == nil && n > 0 && n <= 5000 {
			batchSize = n
		}
	}
	totalIndex := len(indexInfoList)
	if totalIndex == 0 {
		logger.Infof(ctx, "processChunks no index items to write (0 chunks)")
	} else {
		batches := (totalIndex + batchSize - 1) / batchSize
		for i := 0; i < totalIndex; i += batchSize {
			end := i + batchSize
			if end > totalIndex {
				end = totalIndex
			}
			batchNo := (i / batchSize) + 1
			logger.Infof(ctx, "Indexing progress: batch %d/%d items %d-%d/%d", batchNo, batches, i+1, end, totalIndex)
		...
		logger.GetLogger(ctx).Infof("processChunks batch index successfully, with %d index", totalIndex)
	}
...
func (s *knowledgeService) triggerManualProcessing(ctx context.Context,
	...
	var vlmConfig *proto.VLMConfig // 始终为 nil
...
func (s *knowledgeService) ProcessDocument(ctx context.Context, t *asynq.Task) (retErr error) {
	...
	// Best-effort safety net: if any unexpected panic happens during processing,
	// ensure the knowledge doesn't get stuck in "processing" forever.
	var knowledge *types.Knowledge
	defer func() {
		if r := recover(); r != nil {
			logger.Errorf(ctx, "panic recovered in ProcessDocument: %v", r)
			errMsg := fmt.Sprintf("panic during document processing: %v", r)

			k := knowledge
			if k == nil && payload.TenantID != 0 && payload.KnowledgeID != "" {
				if got, err := s.repo.GetKnowledgeByID(ctx, payload.TenantID, payload.KnowledgeID); err == nil {
					k = got
				}
			}
			if k != nil {
				k.ParseStatus = "failed"
				k.ErrorMessage = errMsg
				k.UpdatedAt = time.Now()
				if err := s.repo.UpdateKnowledge(ctx, k); err != nil {
					logger.Errorf(ctx, "failed to update knowledge after panic: %v", err)
				}
			}

			// Return an error so Asynq can retry (if configured).
	...
	// 说明：图片文件在未启用多模态时也允许进入本地解析链路（OCR 等）。
