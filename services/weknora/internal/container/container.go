...
func initDatabase(cfg *config.Config) (*gorm.DB, error) {
	...
	case "postgres":
		...
		// Resolve embeddings migration policy explicitly to avoid accidental retriever switches.
		retrieveDriver := normalizeRetrieveDrivers(os.Getenv("RETRIEVE_DRIVER"))
		skipEmbedding, policy, policyErr := resolveEmbeddingMigrationPolicy(retrieveDriver)
		if policyErr != nil {
			return nil, policyErr
		}
		skipEmbeddingValue := "true"
		if !skipEmbedding {
			skipEmbeddingValue = "false"
		}
		logger.Infof(
			context.Background(),
			"Embedding migration policy=%s, retrieve_driver=%v, skip_embedding=%s",
			policy,
			retrieveDriver,
			skipEmbeddingValue,
		)
	...
	maxOpenConns := parsePositiveIntEnv("DB_MAX_OPEN_CONNS", 20)
	maxIdleConns := parsePositiveIntEnv("DB_MAX_IDLE_CONNS", 10)
	if maxIdleConns > maxOpenConns {
		maxIdleConns = maxOpenConns
	}
	connMaxLifetime := time.Duration(parsePositiveIntEnv("DB_CONN_MAX_LIFETIME_MINUTES", 10)) * time.Minute
	connMaxIdleTime := time.Duration(parsePositiveIntEnv("DB_CONN_MAX_IDLE_TIME_MINUTES", 5)) * time.Minute
	...
	logger.Infof(
		context.Background(),
		"Configured DB pool: max_open=%d max_idle=%d conn_max_lifetime=%s conn_max_idle_time=%s",
		maxOpenConns,
		maxIdleConns,
		connMaxLifetime,
		connMaxIdleTime,
...
func parsePositiveIntEnv(name string, fallback int) int {
	...
	if err != nil || value <= 0 {
		logger.Warnf(context.Background(), "Invalid %s=%q, fallback to %d", name, raw, fallback)
		return fallback
	}
	return value
}

func normalizeRetrieveDrivers(raw string) []string {
...
func resolveEmbeddingMigrationPolicy(retrieveDrivers []string) (bool, string, error) {
	policy := strings.ToLower(strings.TrimSpace(os.Getenv("EMBEDDING_MIGRATION_POLICY")))
	if policy == "" {
		policy = "auto"
	}

	hasPostgres := slices.Contains(retrieveDrivers, "postgres")
	hasQdrant := slices.Contains(retrieveDrivers, "qdrant")

	switch policy {
	case "auto":
		return !hasPostgres, policy, nil
	case "qdrant_only":
		if !hasQdrant {
			return false, policy, fmt.Errorf(
				"EMBEDDING_MIGRATION_POLICY=qdrant_only requires RETRIEVE_DRIVER to include qdrant (current=%v)",
				retrieveDrivers,
			)
		}
		if hasPostgres {
			return false, policy, fmt.Errorf(
				"EMBEDDING_MIGRATION_POLICY=qdrant_only forbids postgres retriever (current=%v)",
				retrieveDrivers,
			)
		}
		return true, policy, nil
	case "postgres_required":
		if !hasPostgres {
			return false, policy, fmt.Errorf(
				"EMBEDDING_MIGRATION_POLICY=postgres_required requires RETRIEVE_DRIVER to include postgres (current=%v)",
				retrieveDrivers,
			)
		}
	...
	default:
		return false, policy, fmt.Errorf(
			"unsupported EMBEDDING_MIGRATION_POLICY=%q (supported: auto|qdrant_only|postgres_required)",
			policy,
		)
	}
}
