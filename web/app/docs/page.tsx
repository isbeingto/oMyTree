...
async function getPublishedDocs(): Promise<Doc[]> {
  try {
    const result = await pool.query(
      `SELECT id, title, slug, summary, lang, updated_at
       FROM site_docs
       WHERE status = 'published' AND lang = 'en' AND doc_type = 'article'
...
export default async function DocsPage() {
  ...
  return (
    <MarketingLayout activeNav="docs">
        ...
        {hasDocs ? (
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="mt-3 flex items-center gap-3 text-xs text-slate-500 dark:text-slate-500">
                        <time dateTime={new Date(doc.updated_at).toISOString()}>{formatDate(doc.updated_at)}</time>
                      </div>
                    </div>
          ...
          <FadeIn delay={0.1}>
            <div className="rounded-2xl glass-panel-strong border border-dashed border-slate-300/50 dark:border-white/10 px-8 py-16 text-center">
              ...
              <h2 className="mt-4 text-lg font-medium text-slate-900 dark:text-white">{mt("en", "docs_empty_title")}</h2>
              <p className="mt-2 text-sm text-slate-600 dark:text-slate-400">
                {mt("en", "docs_empty_desc")}
              </p>
              <p className="mt-4 text-xs text-slate-500 dark:text-slate-500">
                Admin? <Link href="/admin/docs" className="text-emerald-600 dark:text-emerald-400 hover:underline">{mt("en", "docs_empty_admin_link")}</Link>
              </p>
            </div>
          </FadeIn>
        )}
