...
import { isValidLocale, type SiteLocale, localePath } from '@/lib/site-i18n/locale-utils';
...
export default async function LocaleAboutPage({
  ...
  return (
    <MarketingLayout activeNav="about" locale={locale}>
          ...
          <section className="rounded-2xl glass-card glass-card-hover p-8 md:p-10 space-y-6 relative overflow-hidden group">
            ...
            <div className="relative z-10">
              <h2 className="text-sm font-bold uppercase tracking-widest text-emerald-600 dark:text-emerald-400 mb-4">
                {mt(locale, 'about_idea_label')}
              </h2>
              <p className="text-xl md:text-2xl text-slate-800 dark:text-slate-200 leading-relaxed font-medium">
                {mt(locale, 'about_idea_headline').split('过程').length > 1 ? (
                  <>
                    {mt(locale, 'about_idea_headline').split('过程')[0]}
                    <span className="italic text-emerald-600 dark:text-emerald-400">过程</span>
                    {mt(locale, 'about_idea_headline').split('过程')[1]}
                  </>
                ) : (
                  <>
                    {mt(locale, 'about_idea_headline').split('process')[0]}
                    <span className="italic text-emerald-600 dark:text-emerald-400">process</span>
                    {mt(locale, 'about_idea_headline').split('process')[1]}
                  </>
                )}
              </p>
              <p className="text-slate-600 dark:text-slate-400 mt-4 text-lg">
                {mt(locale, 'about_idea_body')}
              </p>

              <div className="grid sm:grid-cols-3 gap-6 mt-10 pt-10 border-t border-slate-200 dark:border-slate-800">
                {([
                  mt(locale, 'about_value_1'),
                  mt(locale, 'about_value_2'),
                  mt(locale, 'about_value_3'),
          ...
          <FadeIn delay={0.3}>
            <section className="h-full rounded-2xl glass-card glass-card-hover p-8 space-y-4 group">
              <h2 className="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
                ...
                {mt(locale, 'about_for_title')}
              </h2>
              <ul className="space-y-3">
                {([
                  mt(locale, 'about_for_1'),
                  mt(locale, 'about_for_2'),
                  mt(locale, 'about_for_3'),
          ...
          <FadeIn delay={0.4}>
            <section className="h-full rounded-2xl glass-card glass-card-hover p-8 space-y-4 group">
              <h2 className="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
                ...
                {mt(locale, 'about_not_title')}
              </h2>
              <ul className="space-y-3">
                {([
                  mt(locale, 'about_not_1'),
                  mt(locale, 'about_not_2'),
                  mt(locale, 'about_not_3'),
                ] as string[]).map((item, i) => (
        ...
        <FadeIn delay={0.6}>
          <div className="text-center py-12 border-t border-slate-200 dark:border-slate-800 relative overflow-hidden">
            ...
            <div className="relative z-10 space-y-2">
              <h2 className="text-sm font-bold uppercase tracking-widest text-slate-500 dark:text-slate-500">
                {mt(locale, 'about_status_label')}
              </h2>
              <p className="text-slate-600 dark:text-slate-400 text-sm max-w-md mx-auto">
                {mt(locale, 'about_status_text')}
              </p>
            </div>
          </div>
        </FadeIn>
